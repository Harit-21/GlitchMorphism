document.addEventListener('DOMContentLoaded', () => {
    ...
    // ✅ ADDED: New API function to clear a timer from view permanently
    ...
    // ✅ CHANGED: Smarter logic for the '✖' button
    timersContainer.addEventListener('click', async e => {
        if (e.target.classList.contains('delete-btn')) {
            ...
            if (timer && timer.remaining_seconds > 0) {
                // If the timer is ACTIVE, ask to DELETE it from the database
                if (confirm(`Are you sure you want to delete the active timer "${timer.name}"?`)) {
                    await apiDeleteTimer(timerId);
                }
            } else if (timer) {
                // If the timer is FINISHED, just CLEAR it (mark as cleared in DB)
    ...
    // ✅ CHANGED: "Clear Finished" button now uses the new API endpoint
    clearFinishedBtn.addEventListener('click', async () => {
        ...
        if (confirm(`Are you sure you want to clear ${finishedTimers.length} finished timer(s) from view? This is permanent.`)) {
    ...
    // --- (All other functions below this line are unchanged and correct) ---
    function renderTimers() {
        ...
        timersState.forEach(timer => {
            ...
            div.innerHTML = `<div class="flex justify-between items-start"><div><h2 class="font-bold text-lg mb-1">${timer.name}</h2><p class="font-mono ${isDone ? 'text-emerald-400' : 'text-slate-300'}">${formatRemaining(timer.remaining_seconds)}</p></div><button data-id="${timer.id}" class="delete-btn text-slate-500 hover:text-red-400 transition">✖</button></div><div class="w-full bg-stone-800 h-2 rounded overflow-hidden"><div class="h-full bg-amber-500 transition-all duration-1000 ease-linear" style="width: ${percent}%"></div></div>`;
